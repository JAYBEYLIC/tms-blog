...
if($stmt->rowCount() < 1){
  ...
  $userinfo = selectContent($conn,'panel_members',$whereHash_id);

  $stmt = $conn->prepare("UPDATE panel_members SET verification=:vs,input_members_status=:us WHERE input_email=:gid");
  $ver = 1;
  $status = 1;
  ...
  $email = $userinfo[0]['input_email'];
  $firstname = $userinfo[0]['input_firstname'];
  $lastname = $userinfo[0]['input_lastname'];
// if($userinfo['level'] == 9 ){
